...
export default class Checkout extends Component {
    ...
    componentWillMount() {
        ...
        Axios
            .get("http://localhost:3001/orders/cart/my/" + id, this.state.config)
            .then((response) => {
    ...
    handleCashOnDelivery=event=>{
        ...
        {
            console.log(response)
            this.setState({billed:response.data._id})
            localStorage.setItem('cart', this.state.Id)

              var proID = localStorage.getItem("productId");
             
              this.setState({
                  Totalqauntity:localStorage.getItem("quantity")
              })
              var data={
               
                  quantity:this.state.Totalqauntity
              }
              
              Axios.put("http://localhost:3001/products/"+proID,data,this.state.config)
              .then((response)=>{
                  console.log(response)
