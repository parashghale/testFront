...
export default class Myproduct extends Component {
    ...
    handleDelete = event => {
        if (confirm("Are you sure to delete this item ?")) {
            ...
            Axios
                .delete("http://localhost:3001/products/" + event, this.state.config)
                .then((response) => {
                    console.log(response.data)
                    this.setState({isDeleted: "Product deleted sucessfully",redirect:true})
        ...
        return (
            <div>
                ...
                <Container>
                    ...
                    <Row>
                        {products.length
                            ? (products.map(product =>< CardTitle className = "col-md-4 col-sm-6" key = {
                                product._id
                            } > <CardBody className="cardImage">
                                <img src={this.state.path + product.image} alt="Card image"/>
                            </CardBody> < CardBody > <CardTitle>{product.productName}</CardTitle> < CardTitle > {
                                product.productCondition
                            } </CardTitle>
                        <CardSubtitle>Price: Rs.{product.productPrice}</CardSubtitle > <CardText>{product.productDescription}</CardText> < CardLink > <Link to={`/dashboard/product/my/${product._id}`}>
